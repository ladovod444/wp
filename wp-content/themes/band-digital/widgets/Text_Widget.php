...
class Text_Widget extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct(
			'text_widget', // ID виджета, если не указать (оставить ''), то ID будет равен названию класса в нижнем регистре: foo_widget
			'Simple text',
			array( 'description' => 'Simple text', 'classname' => 'text', )
		);

		// скрипты/стили виджета, только если он активен
		if ( is_active_widget( false, false, $this->id_base ) || is_customize_preview() ) {
			add_action('wp_enqueue_scripts', array( $this, 'add_text_widget_scripts' ));
			add_action('wp_head', array( $this, 'add_text_widget_style' ) );
		}
	}
	...
	// скрипт виджета
	function add_text_widget_scripts() {
		// фильтр чтобы можно было отключить скрипты
		if( ! apply_filters( 'show_text_widget_script', true, $this->id_base ) )
			return;
	...
	// стили виджета
	function add_text_widget_style() {
		// фильтр чтобы можно было отключить стили
		if( ! apply_filters( 'show_text_widget_style', true, $this->id_base ) )
			return;
		?>
		<style type="text/css">
