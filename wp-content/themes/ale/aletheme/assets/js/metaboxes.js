...
jQuery(document).ready(function ($) {
	...
	$('.ale_upload_button').on('click', function (e) {
        e.preventDefault();
        ...
        mediaFrame.on('select', function () {
            ...
            // Set the value of the input field to the file URL
            $('input[name="' + formfield + '"]').val(attachment.url);
        });
    
        mediaFrame.open();
    });

	$('.ale_remove_file_button').on('click', function () {
    ...
    window.send_to_editor = function (html) {
		...
		if (formfield) {
			...
			if (itemurl.match(image)) {
				uploadStatus = '<div class="img_status"><img src="' + itemurl + '" alt="" /><a href="#" class="ale_remove_file_button" rel="' + formfield + '">Remove Image</a></div>';
			} else {
				// No output preview if it's not an image
				// Standard generic output if it's not an image.
				html = '<a href="' + itemurl + '" target="_blank" rel="external">View File</a>';
				uploadStatus = '<div class="no_image"><span class="file_link">' + html + '</span>&nbsp;&nbsp;&nbsp;<a href="#" class="ale_remove_file_button" rel="' + formfield + '">Remove</a></div>';
			}

			$('#' + formfield).val(itemurl);
			$('#' + formfield + '_id').val(itemid);
			$('#' + formfield).siblings('.ale_upload_status').slideDown().html(uploadStatus);
			tb_remove();
