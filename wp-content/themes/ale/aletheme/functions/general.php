...
function ale_limpieza_page_links() {
	...
	$pagination = array(
		...
		'show_all' => false,
		'type' => 'list',
		'next_text' => '<span>' . esc_html__('Next page', 'ale') . '</span>',
		'prev_text' => '<span>' . esc_html__('Previous', 'ale') . '</span>'
	);
	...
	if($wp_query->max_num_pages > 1){
		echo '<div class="pagination font_one clear">';
		if($current == 1){
			echo '<span class="pseudo-link"><span>' . esc_html__('Previous', 'ale') .'</span></span>';
		}

		echo paginate_links($pagination);

		if($wp_query->max_num_pages == $current){
			echo '<span class="pseudo-link last"><span>' . esc_html__('Next page', 'ale') .'</span></span>';
		}
		echo '</div>';
	}
}
...
function ale_custom_sidebar_widgets_init() {
    ...
    if(ALETHEME_DESIGN_STYLE == 'mimi'){
        register_sidebar(array(
            'name' => 'Shop Sidebar',
            'id' => 'shop-sidebar',
            'description' => esc_html__('Appears as the right sidebar on shop archive pages in case you selected "show sidebar" in theme options panel.','ale'),
    ...
    register_sidebar(array(
        'name' => 'Main Sidebar',
        'id' => 'main-sidebar',
        'description' => esc_html__('Appears as the left (or right) sidebar on blog post in case you selected "show sidebar" in meta data for specific post or theme options panel.','ale'),
        'before_widget' => '<div id="%1$s" class="widget %2$s">',
        'after_widget' => '</div>',
        'before_title' => '<h5 class="widget_title">',
        'after_title' => '</h5>',
    ...
    if(ale_get_option('footer_variant') == 'widget'){
        register_sidebar(array(
            'name' => 'Footer One Sidebar',
            'id' => 'footer-one-sidebar',
            'description' => esc_html__('Appears as the first column sidebar in Footer. You should also enable Widgets Footer in Theme Options.','ale'),
            'before_widget' => '<div id="%1$s" class="widget %2$s">',
            'after_widget' => '</div>',
            'before_title' => '<h5 class="widget_title">',
            'after_title' => '</h5>',
        ...
        register_sidebar(array(
            ...
            'description' => esc_html__('Appears as the second column sidebar in Footer. You should also enable Widgets Footer in Theme Options.','ale'),
            'before_widget' => '<div id="%1$s" class="widget %2$s">',
            'after_widget' => '</div>',
            'before_title' => '<h5 class="widget_title">',
            'after_title' => '</h5>',
        ...
        register_sidebar(array(
            ...
            'description' => esc_html__('Appears as the third column sidebar in Footer. You should also enable Widgets Footer in Theme Options.','ale'),
            'before_widget' => '<div id="%1$s" class="widget %2$s">',
            'after_widget' => '</div>',
            'before_title' => '<h5 class="widget_title">',
            'after_title' => '</h5>',
...
//Functions for Nunta Blog Style
//==============================
function ale_init_js_vars() {
    wp_localize_script(
        ...
        'ale',
        array(
            'template_dir'      => ALETHEME_THEME_URL,
            'ajax_load_url'     => site_url('/wp-admin/admin-ajax.php'),
            'expand_text'       => esc_html__('Expand Story','ale'),
            'close_text'       => esc_html__('Close Story','ale'),
        )
    );
}
add_action('wp_print_scripts', 'ale_init_js_vars');

function ale_load_nunta_post() {
...
add_action('wp_ajax_aletheme_nunta_load_post', 'ale_load_nunta_post');
add_action('wp_ajax_nopriv_aletheme_nunta_load_post', 'ale_load_nunta_post');
...
function ale_get_breadcrumbs() {
    ...
    $text['home']     = esc_html__('Home','ale');
    $text['category'] = esc_html__('Archive','ale').' "%s"';
    $text['search']   = esc_html__('Search results','ale').' "%s"';
    $text['tag']      = esc_html__('Tag','ale').' "%s"';
    $text['author']   = esc_html__('Author','ale').' %s';
    $text['404']      = esc_html__('Error 404','ale');
    ...
    if (is_home() || is_front_page()) {
        ...
        if(get_option( 'page_for_posts' )){
                echo '<div class="breadcrumbs"><a href="' . esc_url($home_link) . '">' . esc_attr($text['home']) . '</a>'.ale_wp_kses($delimiter).' '.__('Blog','ale').'</div>';
        }
    }
    else {
        ...
        if ( get_query_var('paged') ) {
            ...
            echo ale_wp_kses($delimiter) . esc_html__('Page','ale') . ' ' . get_query_var('paged');
            if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo ')';
        }
...
function ale_register_required_plugins() {
        ...
        $plugins = array(
            ...
            array(
                'name'                  => 'Ale Theme Core',
                'slug'                  => 'cpt-aletheme',
                'source'                => get_template_directory() . '/plugins/cpt-aletheme.zip',
                'required'              => true,
                'version'               => '1.1.1',
...
function ale_body_post_classes( $classes) {
        ...
        if(ale_get_option('design_variant')=='taxipress'){
            if(is_front_page()){
                $classes[] = "home";
            } else {
                $classes[] = "all";
...
//Posts per page for works post type
function ale_set_posts_per_page_for_works( $query ) {
    $ale_works_posts_per_page = '';

    if(ale_get_option('works_posts_per_page')){
        $ale_works_posts_per_page = ale_get_option('works_posts_per_page');
    }

    if(!empty($ale_works_posts_per_page)){
        if ( !is_admin() && $query->is_main_query() && is_post_type_archive( 'works' ) ) {
        $query->set( 'posts_per_page', $ale_works_posts_per_page );
        }
    }
}
add_action( 'pre_get_posts', 'ale_set_posts_per_page_for_works' );
