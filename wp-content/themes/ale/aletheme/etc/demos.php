...
class ale_demo_content extends ale_demo_base {
    ...
    static function add_post($params) {
        ...
        if(!empty($params['ale_video_link'])){
            update_post_meta($post_id, 'ale_video_link', $params['ale_video_link']);
        }
        if(!empty($params['ale_author'])){
            update_post_meta($post_id, 'ale_author', $params['ale_author']);
        }
        if(!empty($params['ale_year'])){
            update_post_meta($post_id, 'ale_year', $params['ale_year']);
        }
        if(!empty($params['ale_location'])){
            update_post_meta($post_id, 'ale_location', $params['ale_location']);
        }
    ...
    static function add_page($params) {
        ...
        if(!empty($params['ale_post_heading_icon'])){
            update_post_meta($page_id, 'ale_post_heading_icon', $params['ale_post_heading_icon']);
        }
...
class ale_demo_options extends ale_demo_base {
    ...
    static function update_shop_page($params) {
        if(!empty($params)) {
            update_option('woocommerce_shop_page_id', $params);
        }
    }
...
class ale_demo_installer {
    ...
    public function import_panel_settings($file_path) {
        ...
        foreach ($file_settings as $setting_id => $setting_value) {
            ...
            if(in_array($setting_id, array('ale_workpagebg','ale_sitelogo','ale_sitelogoretina','ale_footerlogo','ale_footerlogoretina','ale_archivetitlebg','ale_wootitlebg','ale_woosingletitlebg','ale_footerbg','ale_map_icon','ale_worktitlebg'))) {
