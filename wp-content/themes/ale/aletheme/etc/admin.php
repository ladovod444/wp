...
function ale_add_scripts($hook) {
    ...
    if (in_array($hook, array('ale-dashboard_page_aletheme'))) {
    ...
    if (in_array($hook, array('ale-dashboard_page_aletheme_theme_demos'))) {
        wp_enqueue_style('font-awesome');
        ...
        wp_localize_script( 'aletheme-demos', 'olins_strings', array(
            'installingPlugin' => esc_html__( 'Installing plugin', 'ale' ),
            'activatingPlugin' => esc_html__( 'Activating plugin', 'ale' ),
	        'tryAgain' => esc_html__('Try again', 'ale'),
            ...
            'plugin_failed_activation' => esc_html__( 'Failed to activate.', 'ale' ),
            'plugin_active' => esc_html__( 'Active', 'ale' ),
            'plugin_failed_activation_retry' => esc_html__( 'Failed.', 'ale' ),
            'plugin_failed_activation_memory' => esc_html__( 'The plugin failed to activate. Please try to increase the memory_limit on your server.', 'ale' ),
            'content_importing_error' => esc_html__( 'There was a problem during the importing process resulting in the following error from your server:', 'ale' ),
            'required_plugins' => esc_html__('Install required plugins first.', 'ale'),
        ) );
    }
...
function aletheme_welcome(){
    require_once "admin_templates/ale_view_welcome.php";
}
...
function ale_add_admin_menu() {
    add_menu_page(esc_html__('Ale Welcome', 'ale'), esc_html__('Ale Dashboard', 'ale'), 'manage_options', 'ale_welcome','aletheme_welcome','dashicons-dashboard',60);
    add_submenu_page('ale_welcome', esc_html__('Theme Options for Ale', 'ale'), esc_html__('Theme Options', 'ale'), 'manage_options', 'aletheme','optionsframework_page',);
    add_submenu_page('ale_welcome', esc_html__('Import demos for Ale', 'ale'), esc_html__('Demo Import', 'ale'), 'manage_options', 'aletheme_theme_demos', 'ale_theme_demos');
}
...
function ale_admin_posts_featured_column_content($column_name, $post_id) {
	if ($column_name == 'featured-image') {  
		...
		if ($post_featured_image) {  
			echo '<img src="' . esc_url($post_featured_image) . '" alt="'.esc_html__('Image','ale').'" width="60" />';
		}  
	}
}
...
function ale_admin_posts_first_image_column_content($column_name, $post_id) {
	if ($column_name == 'first-image') {  
		...
		if ($image) {  
			echo '<img src="' . esc_url($image) . '" alt="'.esc_html__('Image','ale').'" width="60" />';
		}  
	}
}
...
function ale_ajax_post_install_plugin() {
    ...
    if ( $plugin_installed === true ) {
        ...
        echo sprintf( __( 'The plugin failed to install. Please check the permissions for the "plugins" directory or <a href="%s" target="_blank">download</a> the plugin and install it manually.', 'ale' ), $plugin_source );
    }
    die();
}
