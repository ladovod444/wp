...
if ( is_admin() ) {
	...
	$of_page= 'ale-dashboard_page_aletheme';
...
if ( ! function_exists( 'optionsframework_medialibrary_uploader' ) ) {
	...
	function optionsframework_medialibrary_uploader( $_id, $_value, $_mode = 'full', $_desc = '', $_postid = 0, $_name = '') {
		...
		if(function_exists('optionsframework_mlu_init')){
			$output .= '<input id="upload_' . $id . '" class="upload_button button" type="button" value="' . esc_html__( 'Upload', 'ale' ) . '" rel="' . $int . '" />' . "\n";
		} else {
			$output .= '<span class="noteplugin">For upload options you should install the <strong>Ale Theme Core</strong> plugin.</span>' . "\n";
		}
		if ( $_desc != '' ) {
		...
		if ( $value != '' ) { 
			...
			if ( $image ) {
				$output .= '<img src="' . $value . '" alt="'.esc_html__('Image','ale').'" />'.$remove.'';
			} else {
				$parts = explode( "/", $value );
				...
				$title = esc_html__( 'View File', 'ale' );
...
if ( ! function_exists( 'optionsframework_mlu_modify_tabs' ) ) {
	...
	function optionsframework_mlu_modify_tabs ( $tabs ) {
		$tabs['gallery'] = str_replace( esc_html__( 'Gallery', 'ale' ), esc_html__( 'Previously Uploaded', 'ale' ), $tabs['gallery'] );
		return $tabs;
	}
}