...
?>
<?php if(ALETHEME_DESIGN_STYLE == 'keira'){ ?>
    <div class="title_background_container">
        <div class="page_title_container <?php echo esc_attr($ale_bg_for_title_class); ?> cf"  data-scroll-speed="4" data-scroll-type="background" <?php if($ale_bg_for_title){ ?>style="background-image: url(<?php echo esc_url($ale_bg_for_title); ?>);"<?php } ?>>
            <div class="mask"></div>
            <div class="page_title cf" data-scroll-speed="-2" data-scroll-type="element">
                <?php if($ale_title_for_heading){ ?><h2 <?php if(ALETHEME_DESIGN_STYLE == 'rosi'){echo 'class="font_three"';} ?>><?php echo esc_html($ale_title_for_heading); ?></h2><?php } ?>
                <span class="tagline font_two">
                     <?php echo esc_attr(get_bloginfo('description')); ?>
                </span>
                <?php if($ale_pre_title_for_heading){ ?><p class="font_two"><?php echo esc_html($ale_pre_title_for_heading); ?></p><?php } ?>
...
<div class="title_background_container">
	<div class="page_title_container <?php echo esc_attr($ale_bg_for_title_class); ?> cf"  data-scroll-speed="4" data-scroll-type="background" <?php if($ale_bg_for_title){ ?>style="background-image: url(<?php echo esc_url($ale_bg_for_title); ?>);"<?php } ?>>
		...
		<div class="page_title cf" data-scroll-speed="-2" data-scroll-type="element">
			<?php
			//Open Wrapper for specific demos
			if(in_array(ALETHEME_DESIGN_STYLE, array('rosi','burger','moka'))){ echo '<div class="content_wrapper">';} ?>
			
			<?php if($ale_pre_title_for_heading){ ?><span class="font_two"><?php echo esc_html($ale_pre_title_for_heading); ?></span><br /><?php } ?>
			<?php if($ale_title_for_heading){ ?><h2 <?php if(ALETHEME_DESIGN_STYLE == 'rosi'){echo 'class="font_three"';} ?>><?php echo esc_html($ale_title_for_heading); ?></h2><?php } ?>
			
			<?php 
			//Close wrapper for specific demos
			if(in_array(ALETHEME_DESIGN_STYLE, array('rosi','burger','moka'))){ echo '</div>';} ?>
		</div>
	</div>
</div>
<?php } ?>