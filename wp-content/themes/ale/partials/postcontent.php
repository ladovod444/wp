<div class="story cf">
    ...
    <?php if(ale_get_meta('featured_position') == 'in_content' or ale_get_meta('featured_position') == ''){
        if(get_the_post_thumbnail($post->ID,'full')){
            if(intval(ale_get_option('inner_wrapper')) > 1100){
                echo '<div class="single_featured_image">'.get_the_post_thumbnail($post->ID,'full').'</div>';
            } else {
    ...
    <?php if(ale_get_meta('post_title_position') == 'afterheading' or ale_get_meta('post_title_position') == "" or ale_get_meta('post_title_position') == 'afterheadingwithdefaults' or ale_get_meta('post_title_position') == 'fullwidthwedding'){ ?>
        ...
        <?php if(ale_get_meta('post_pre_title')){ ?>
            <p class="pre_title"><?php echo esc_html(ale_get_meta('post_pre_title')); ?></p>
        <?php } ?>
    <?php } ?>
    ...
    if ( $images ) {
        if(has_post_format('gallery',$post->ID) or is_singular('works')){
            if(ale_get_meta('gallery_slider') == 'top_full' or ale_get_meta('gallery_slider') == 'brigitte_slider' or ale_get_meta('gallery_slider') == 'orquidea_slider'){
    ...
    <?php }  else {
        if(ale_get_option('blog_grid_layout') == 'nunta'){?>
        <section class="full">
    ...
    <?php if(get_the_tags()){ ?>
        <p class="tagsphar"><?php the_tags( esc_html__('Tags: ','ale'), ' ', '<br />' );  ?></p>
    <?php } ?>
    ...
    <?php if(ale_get_option('blog_share_icons') == 'show'){?>
        <?php if(function_exists('ale_get_share')){ ?>
            <div class="single_post_share">
                <span class="label"><?php echo esc_html_e('Share this post, choose your Platform!','ale') ?></span>
